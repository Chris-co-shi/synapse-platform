...
| `synapse-mybatis-plus` | MyBatis-Plus 工程增强 | 已声明并用于 IAM 持久化适配 |
| `synapse-datasource` | 数据源治理 | 已声明并复用官方 dynamic-datasource 配置模型 |
...
IAM 使用 PostgreSQL，并由 Flyway 管理 schema 演进。当前持久化基线仅创建 `iam_user` 与 `iam_user_credential`，角色、权限、客户端和 Refresh Token 表仍属于后续阶段。

- 表名、字段名统一使用小写 snake_case，并采用稳定的 IAM 领域前缀。
- 主键使用 MyBatis-Plus `ASSIGN_ID` 生成并以 `String`/`varchar(19)` 保存，避免向 API 暴露数据库数值类型。
- 可并发修改的聚合使用乐观锁；审计字段记录创建、修改时间和操作主体。
- 软删除只用于确有恢复、留痕或引用完整性需求的数据，不替代状态机和审计记录；当前两张基础表不启用逻辑删除。
- 用户名执行 trim、Unicode NFKC 和 `Locale.ROOT` 小写规范化，数据库唯一约束作用于 `normalized_username`。
...
- Entity、Mapper、Repository Adapter 只存在于 Server，不通过 IAM API DTO 暴露，也不供其他服务共享数据库访问。
...
- Framework WebMVC、Security、OAuth2、Cache、Audit、Data、MyBatis-Plus 和 DataSource 技术依赖声明；
- PostgreSQL 驱动、Druid、Flyway Core 与 PostgreSQL 数据库支持依赖；
- `V1__create_iam_identity_tables.sql`，创建用户与认证凭据两张基础表、唯一约束、外键和检查约束；
- 用户、用户状态、认证凭据领域模型及 Repository Port；
- MyBatis-Plus Entity、Mapper 与 Repository Adapter，领域层不继承 ORM 类型；
- 用户名规范化策略与单元测试；
- PostgreSQL Testcontainers 集成测试代码，覆盖空库迁移、规范化查询、唯一约束和乐观锁。

上述持久化能力仍需在可访问 Framework 构件和 Docker 的本地或 CI 环境中执行完整 Maven 验证后，才可视为通过 IAM-02 验收。

### 尚未完成

- 角色、权限和 OAuth2 Client 领域模型及持久化；
- PasswordEncoder 接入、密码认证和账户状态策略；
- 登录失败计数、临时锁定和认证审计应用服务；
...
- 当前状态：本分支已实现代码，等待本地 Maven 与 Docker/Testcontainers 审核。
