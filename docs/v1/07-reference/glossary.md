...
Identity and Access Management。负责主体、凭据、认证、OAuth 2.0 / OIDC、Opaque Token、Client、会话、角色授权关系和 Redis 授权快照生命周期。
...
OpenID Connect。建立在 OAuth 2.0 之上的身份认证协议。V1 中需要签名的 ID Token 使用 RS256；Access Token 不使用 JWT。
...
OAuth 2.0 中用于机器客户端或服务身份的授权方式。V1 的 Client Token 同样使用 Opaque Access Token + Redis 授权快照，并与用户身份明确区分。
