...
- 后台任务和纯服务调用目标使用 Client Credentials Access Token；
- 服务身份不得伪装成用户身份；
...
当前标准 Client Credentials 尚未实现，属于 OAuth2 后续计划。当前文档中的 Client Credentials 描述是 V1 目标，不代表已可通过 `/oauth2/token` 获取标准 Client Token。
...
Config 不保存数据库密码、Redis 密码、GatewayProof Secret、OAuth2/OIDC 私钥或 Nacos 技术配置。
