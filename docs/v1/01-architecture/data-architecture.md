...
具体数据库与持久化编码规则见 [`../02-specification/database-conventions.md`](../02-specification/database-conventions.md)。
...
- 禁止跨 Schema SQL；
- 禁止跨服务数据库外键；
- 禁止共享业务 Entity、Mapper 和 Repository；
- 禁止通过视图或存储过程绕过服务边界。

共用实例只是 V1 部署简化策略，不阻碍未来迁移到独立数据库实例。

## 4. Persistence Entity Baseline

Platform 使用 Framework `synapse-mybatis-plus` 已提供的持久化实体基类：
...
- `IdEntity`：`String id` 和 MyBatis-Plus `ASSIGN_ID`；
- `CreatedEntity`：`createdAt`、`createdBy`；
- `MutableEntity`：`updatedAt`、`updatedBy`；
- `VersionedEntity`：`revision` 乐观锁；
- `ManagedEntity`：`deleted` 逻辑删除。

约束：

- 只有持久化 Entity 可以继承这些基类；
- Domain Model、API DTO 和事件契约不得继承；
- Entity 必须选择满足实际生命周期的最浅基类；
- Platform 不重复定义 Framework 已提供的字段和注解；
- 关系表、日志、Outbox、会话和执行记录不强制继承基类；
- `revision` 是技术并发版本，不是业务修订号。

V1 默认实体主键为 Java `String`，PostgreSQL 使用 `varchar(19)`。
...
- 后续标准 Client Credentials 客户端重新申请 Access Token；
...
## 14. Remaining Module Decisions

以下内容仍由模块设计确认：

- 具体实体选择哪一级 Framework 基类；
- 数据保留、归档和分区策略；
- 个人信息加密与脱敏字段清单；
- 特殊表的联合主键或例外主键策略；
