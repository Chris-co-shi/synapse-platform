...
不能因为系统名称是 MES 或 WMS，就假设它会使用 Framework、Synapse IAM、Synapse 私有 Token、Manifest 或统一权限模型。

## 3. 标准协议优先

跨组织和跨技术栈接入优先使用 OAuth 2.0、OpenID Connect、Bearer Token 和标准 HTTP 契约。
...
V1 优先完成：用户登录、服务调用、第三方 Client Credentials、Token 验证、权限和审计主体。

当前已实现的是自定义 IAM 登录 API + Opaque Token 会话体系；标准 OAuth2/OIDC 仍为计划实现，不能写成已完成。
