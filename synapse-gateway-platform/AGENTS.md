...
- Gateway 只验证 Token 合法性，不判断业务 permission。
- 禁止配置 `hasAuthority`、`hasRole` 或 Route 权限 metadata。
- 下游服务负责接口、资源、组织、租户和数据权限。

## 3. 安全要求

- 转发前清理全部外部 `X-Synapse-Gateway-*` Header。
- GatewayProof Header、canonicalization、query、token hash、nonce 和 HMAC 必须复用 Framework 协议，禁止自行实现。
...
- Docker Compose、构建、部署和回滚细节以 [Gateway Docker 部署](../deploy/docker/gateway/README.md) 为准。
